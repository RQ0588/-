//
//  NPYCommentsInfoViewController.m
//  牛品云
//
...
@interface NPYCommentsInfoViewController () <SDTimeLineCellDelegate, UITextFieldDelegate ,UITableViewDelegate, UITableViewDataSource>
...
@implementation NPYCommentsInfoViewController {
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    self.navigationItem.title = @"朋友圈评论信息通知";
...
#pragma mark -
    ...
    NSDictionary *requeatDict = [NSDictionary dictionaryWithObjectsAndKeys:[dic valueForKey:@"sign"],@"sign",model.user_id,@"user_id",self.moments_id,@"moments_id", nil];
    
    [self requestMomentDataWithUrlString:@"/index.php/app/Moments/get_one" withParame:requeatDict];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell.moreButtonClickedBlock) {
        [cell setMoreButtonClickedBlock:^(NSIndexPath *indexPath) {
            SDTimeLineCellModel *model = weakSelf.model;
...
- (void)requestMomentDataWithUrlString:(NSString *)urlStr withParame:(NSDictionary *)parame {
    ...
    [[NPYHttpRequest sharedInstance] getWithUrlString:[NSString stringWithFormat:@"%@%@",BASE_URL,urlStr] parameters:paremes success:^(id responseObject) {
        ...
        if ([dataDict[@"r"] intValue] == 1) {
            ...
            SDTimeLineCellModel *tpModel = [SDTimeLineCellModel new];
            tpModel.moments_id = [tpDict valueForKey:@"moments_id"];
            tpModel.user_id = [tpDict valueForKey:@"user_id"];
            tpModel.name = [tpDict valueForKey:@"user_name"];
            tpModel.iconName = [tpDict valueForKey:@"user_portrait"];
            tpModel.msgContent = [tpDict valueForKey:@"text"];
            tpModel.picNamesArray = [NSArray arrayWithObjects:[tpDict valueForKey:@"img1"],[tpDict valueForKey:@"img2"],[tpDict valueForKey:@"img3"], nil];
            tpModel.time = [tpDict valueForKey:@"time"];
            ...
            tpModel.commentItemsArray = [tempComments copy];
...
- (SDTimeLineCellModel *)model {
    if (_model == nil) {
        _model = [SDTimeLineCellModel new];
    }
